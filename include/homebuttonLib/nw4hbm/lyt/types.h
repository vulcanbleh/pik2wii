...
{
	template <typename T>
	inline T const *ConvertOffsToPtr(void const *baseAddress,
	                                 unsigned int offset)
	{
		return reinterpret_cast<T const *>(
			reinterpret_cast<unsigned int>(baseAddress) + offset);
	}

	template <typename T>
	inline T *ConvertOffsToPtr(void *baseAddress, unsigned int offset)
	{
		return reinterpret_cast<T *>(reinterpret_cast<unsigned int>(baseAddress)
		                             + offset);
	}
