 ...
 ******************************************************************************/
struct Material {
    char name[NW4R_LYT_MATERIAL_NAME_LEN]; // _00
    GXColorS10 tevCols[TEVCOLOR_MAX];      // _14
    GXColor tevKCols[GX_MAX_KCOLOR];       // _2C
    MaterialResourceNum resNum;            // _3C
};
...
public:
	...
	virtual ~Material(); 														// _08
    virtual bool SetupGX(bool modulate, u8 alpha); 								// _0C
    virtual void BindAnimation(AnimTransform* pAnimTrans);   					// _10
    virtual void UnbindAnimation(AnimTransform* pAnimTrans); 					// _14
    virtual void UnbindAllAnimation();                       					// _18
    virtual void Animate(); 													// _1C
    virtual AnimationLink *FindAnimationLink(AnimTransform *pAnimTrans);        // _20
    virtual AnimationLink *FindAnimationLink(const AnimResource &animRes);      // _24
    virtual void SetAnimationEnable(AnimTransform* pAnimTrans,
                                    bool enable); 								// _28
	virtual void SetAnimationEnable(const AnimResource &animRes, bool enable);  // _2C
...
protected:
    AnimationLinkList mAnimList; 				 // _04
    GXColorS10 mTevCols[TEVCOLOR_MAX]; 			 // _10
    ut::Color mTevKCols[GX_MAX_KCOLOR]; 		 // _28
    detail::BitGXNums mGXMemCap; 				 // _38
    detail::BitGXNums mGXMemNum; 				 // _3C
    void* mpGXMem;               				 // _40
    char mName[NW4R_LYT_MATERIAL_NAME_LEN + 1];  // _44
    bool mbUserAllocated;                        // _59
    u8 PADDING_0x5A[0x5C - 0x5A]; 				 // _5A
