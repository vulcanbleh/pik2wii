...
/**
 * @brief Plain Old Data type for 2D vector.
 *
 * Can be upcasted to real structure. Forces use of lwz/stw.
 *
 * @note Size: 0x8.
 */
struct _VEC2 {
...
struct VEC2 : public _VEC2 {
	...
	VEC2 operator+(const VEC2& rRhs) const { return VEC2(x + rRhs.x, y + rRhs.y); }
	VEC2 operator-(const VEC2& rRhs) const { return VEC2(x - rRhs.x, y - rRhs.y); }
	...
	bool operator==(const VEC2& rRhs) const { return x == rRhs.x && y == rRhs.y; }
	bool operator!=(const VEC2& rRhs) const { return x != rRhs.x || y != rRhs.y; }
...
/**
 * @brief Plain Old Data type for 3D vector.
 *
 * Can be upcasted to real structure. Forces use of lwz/stw.
 *
 * @note Size: 0xC.
...
struct VEC3 : public _VEC3 {
	...
	bool operator==(const VEC3& rRhs) const { return x == rRhs.x && y == rRhs.y && z == rRhs.z; }
	bool operator!=(const VEC3& rRhs) const { return x != rRhs.x || y != rRhs.y || z != rRhs.z; }
...
/**
 * @brief Plain Old Data type for 3x3 matrix.
 *
 * Can be upcasted to real structure. Forces use of lwz/stw.
 *
 * @note Size: 0x24.
...
/**
 * @brief Plain Old Data type for 3x4 matrix.
 *
 * Can be upcasted to real structure. Forces use of lwz/stw.
 *
 * @note Size: 0x30.
...
/**
 * @brief Plain Old Data type for 4x4 matrix.
 *
 * Can be upcasted to real structure. Forces use of lwz/stw.
 *
 * @note Size: 0x40.
...
struct _MTX44 {
	union {
		...
		f32 m[4][4];  // as matrix
		f32 a[4 * 4]; // as single array
		Mtx44 mtx;    // as sdk type
	};
};
...
/**
 * @brief Plain Old Data type for quaternion.
 *
 * Can be upcasted to real structure. Forces use of lwz/stw.
 *
 * @note Size: 0x10.
...
struct QUAT : public _QUAT {
	...
	// NB: these are not real according to kiwi from OGWS, but they're convenient
	operator Quaternion*() { return reinterpret_cast<Quaternion*>(this); }
	operator const Quaternion*() const { return reinterpret_cast<const Quaternion*>(this); }
...
// VEC3
inline VEC3* VEC3Add(register VEC3* pOut, register const VEC3* pA, register const VEC3* pB)
{
...
inline VEC3* VEC3Lerp(register VEC3* pOut, register const VEC3* pVec1, register const VEC3* pVec2, register f32 t)
{
	register f32 work0, work1, work2;
...
inline VEC3* VEC3Scale(register VEC3* pOut, register const VEC3* pIn, register f32 scale)
{
...
inline VEC3* VEC3Sub(register VEC3* pOut, register const VEC3* pA, register const VEC3* pB)
{
	register f32 work0, work1, work2;
