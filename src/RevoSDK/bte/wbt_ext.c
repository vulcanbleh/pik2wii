...
#include "RevoSDK/bte/hcidefs.h"  // LMP_COMPID_BROADCOM
...
BOOLEAN WBT_ExtCreateRecord(void) {
    ...
    temp_u16[0] = 0;
    temp_u16[1] = 1;
    SDP_AddAttribute(sdp_record_handle, ATTR_ID_EXT_BRCM_VERSION, UINT_DESC_TYPE, ARRAY_SIZE(temp_u16), temp_u16);
...
BOOLEAN WBT_ExtAddPinCode(void) {
    ...
    if (SDP_GetLocalDiRecord(&get_device_info, &sdp_record_handle) == SDP_SUCCESS) {
        ...
        SDP_AddAttribute(sdp_record_handle, ATTR_ID_EXT_PIN_CODE, 1, ARRAY_SIZE(temp_u32), temp_u32);
