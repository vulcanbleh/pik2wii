#include "RevoSDK/bte/hidd_api.h"
...
#include "RevoSDK/bte/wcassert.h"  // WCAssert_Line

#include "RevoSDK/bte/btm_api.h"
#include "RevoSDK/bte/gki.h"
#include "RevoSDK/bte/hidd_int.h"
...
UINT32 HID_DevSetSDPRecord(tHID_DEV_SDP_INFO* p_sdp_info) {
    ...
    SDP_AddProtocolList(sdp_handle, ARRAY_SIZE(protocol_list), protocol_list);
...
tHID_STATUS HID_DevSetPowerMgmtParams(UINT8 conn_substate, tHID_DEV_PWR_MD pm_params) {
    if (conn_substate > (int)ARRAY_SIZE(hd_cb.pm_params) - 1)
        return HID_ERR_INVALID_PARAM;
