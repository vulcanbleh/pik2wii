...
static s32 SCReloadConfFileAsync(u8* buf, u32 size, SCReloadConfFileCallback callback);
...
static u32 ParseConfBuf(u8* conf, u32 size);
static BOOL ParseGetBEValue(const u8* start, const u8* end, u32* out, u32 size);
static BOOL UnpackItem(const u8* raw, SCItem* item);
...
u32 ParseConfBuf(u8* bufp, u32 bufSize) {
    ...
    while (tblp < tblEndp && (name = tblp->name) != NULL) {
        ...
        for (loopItem = 0; loopItem < numItems; loopItem++) {
            ...
            if (nameLen == (u32)(((*p) & 0b00011111) + 1) && memcmp(name, p + sizeof(SCType), nameLen) == 0) {
...
static BOOL UnpackItem(const u8* bufp, SCItem* itemp) {
    ...
    itemp->nameLen = (u32)((*bufp & 0b00011111) + 1);
...
void DeleteItemByID(SCItemID id) {
    ...
    if (id < ItemIDMaxPlus1 && ItemIDOffsetTblOffset != 0) {
        ...
        if (targetRef != 0 && ItemNumTotal != 0) {
            itemOfsTop = ((SCConfHeader*)conf)->itemOffsets;
            itemOfsTargetp = (u16 *)(conf + targetRef);
            itemOfsEndp = itemOfsTop + ItemNumTotal;
            initialTopOfFreeSpace = *itemOfsEndp;
            shrinkSize = sizeof(u16) + (itemOfsTargetp[1] - itemOfsTargetp[0]);

            moveSize = *itemOfsTargetp - (targetRef + sizeof(u16));
            memmove(conf + targetRef, conf + targetRef + sizeof(u16), moveSize);
...
BOOL SCReplaceIntegerItem(const void* data, SCItemID id, SCType type) DECOMP_DONT_INLINE {
    SCItem item;
    BOOL result = FALSE;
    BOOL enabled = OSDisableInterrupts();
...
void SCFlushAsync(SCFlushCallback callback) {
    ...
    if (status == SC_STATUS_OK) {
        ...
        if (!__SCIsDirty()) {
            ...
            if (NANDPrivateGetTypeAsync(ConfFileName, &ctrl->nandType,
                                        MyNandCallback, &ctrl->nandCommandBlock) !=
                NAND_RESULT_OK) {
                ErrorFromFlush();
            }
        }
...
#pragma unused(block)
    ...
    case NAND_CB_STATE_0:
        if (result == NAND_RESULT_OK && ctrl->nandType == NAND_FILE_TYPE_FILE) {
            ctrl->nandCbState = NAND_CB_STATE_1;

            if (NANDPrivateGetStatusAsync(
                    ConfFileName, &ctrl->nandStatus, MyNandCallback,
                    &ctrl->nandCommandBlock) != NAND_RESULT_OK) {
                goto _error;
            }
        } else {
            goto _case_1_lbl;
    ...
    case NAND_CB_STATE_1:
        if (result == NAND_RESULT_OK &&
            ctrl->nandStatus.perm == NAND_PERM_RWALL) {
            goto _case_5_lbl;
        }
    _case_1_lbl:
    ...
    case NAND_CB_STATE_2:
        ...
        if (NANDPrivateGetTypeAsync(ConfDirName, &ctrl->nandType,
                                    MyNandCallback,
                                    &ctrl->nandCommandBlock) != NAND_RESULT_OK) {
            goto _error;
        }
    ...
    case NAND_CB_STATE_3:
        if (result == NAND_RESULT_OK && ctrl->nandType == NAND_FILE_TYPE_DIR) {
            goto _case_4_lbl;
        }
    ...
    _case_5_lbl:
        ...
        if (NANDPrivateOpenAsync(ConfFileName, &ctrl->nandFileInfo,
    ...
    case NAND_CB_STATE_6:
        ...
        if (NANDWriteAsync(&ctrl->nandFileInfo, ConfBufForFlush, ctrl->flushSize,
                           MyNandCallback,
                           &ctrl->nandCommandBlock) != NAND_RESULT_OK) {
            goto _error;
        }
    ...
    case NAND_CB_STATE_7:
        ...
        if (NANDCloseAsync(&ctrl->nandFileInfo, MyNandCallback,
                           &ctrl->nandCommandBlock) != NAND_RESULT_OK) {
            goto _error;
        }
        return;
...
static void FinishFromFlush(void) {
    ...
    ctrl = &Control;
    if (ctrl->flushResult != SC_STATUS_OK) {
        __SCSetDirtyFlag();
    }
...
static void ErrorFromFlush(void) {
    ...
    ctrl = &Control;
    ctrl->flushResult = SC_STATUS_FATAL;
