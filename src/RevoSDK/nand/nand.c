...
s32 nandCreate(const char *path, const u8 perm, const u8 attr,
  ...
  if (!privilege_flag && nandIsPrivatePath(absPath)) {
    ...
    if (async_flag) {
        return ISFS_CreateFileAsync(absPath, attr, owner, group,
...
static s32 nandGetFileStatus(s32 fd, u32* lengthOut, u32* positionOut) {
    FSFileStats stats ATTRIBUTE_ALIGN(32);
    s32 result;

    result = ISFS_GetFileStats(fd, &stats);
    if (result == IPC_RESULT_OK) {
        if (lengthOut) {
            *lengthOut = stats.size;
        }

        if (positionOut) {
            *positionOut = stats.offset;
        }
    }
