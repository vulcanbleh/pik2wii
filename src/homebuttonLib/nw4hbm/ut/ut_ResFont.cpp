...
bool ResFont::SetResource(void *brfnt)
{
	...
	if (fileHeader->signature == SIGNATURE_FONT_UNPACKED)
	{
		...
		blockHeader = POINTER_ADD_TYPE(BinaryBlockHeader *, fileHeader,
		...
		while (nBlocks < fileHeader->dataBlocks)
		{
			...
			{
				pFontInfo = POINTER_ADD_TYPE(FontInformation *, blockHeader,
				                             sizeof *blockHeader);

				break;
			}

			blockHeader = POINTER_ADD_TYPE(BinaryBlockHeader *, blockHeader,
			                               blockHeader->size);
			++nBlocks;
		}
	}
	else
	{
		...
		{
			if (!IsValidBinaryFile(fileHeader, SIGNATURE_FONT,
			                       NW4HBM_FILE_VERSION(1, 2), 2))
			{
				return false;
			}
		}
...
FontInformation *ResFont::Rebuild(BinaryFileHeader *fileHeader)
{
	...
	while (nBlocks < fileHeader->dataBlocks)
	{
		...
		{
			info = POINTER_ADD_TYPE(FontInformation *, blockHeader,
			                        sizeof *blockHeader);
		...
		{
			FontTextureGlyph *glyph = POINTER_ADD_TYPE(
		...
		{
			FontWidth *width = POINTER_ADD_TYPE(FontWidth *, blockHeader,
			                                    sizeof *blockHeader);
		...
		{
			FontCodeMap *map = POINTER_ADD_TYPE(FontCodeMap *, blockHeader,
			                                    sizeof *blockHeader);
