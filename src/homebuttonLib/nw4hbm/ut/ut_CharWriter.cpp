...
static void SetupGXCommon()
{
	...
	GXSetChanCtrl(GX_COLOR1A1, false, GX_SRC_REG, GX_SRC_REG, GX_LIGHT_NULL,
	              GX_DF_NONE, GX_AF_NONE);
	GXSetNumTexGens(1);
	GXSetTexCoordGen(GX_TEXCOORD0, GX_TG_MTX3X4, GX_TG_TEX0, 60);
	GXSetNumIndStages(0);
	...
	, mFont(nullptr)
{
	...
	SetTextColor(DEFAULT_COLOR_MAPPING_MAX);
	SetScale(1.0f, 1.0f);
...
void CharWriter::PrintGlyph(f32 x, f32 y, f32 z, Glyph const &glyph)
{
	...
	u16 texRight = static_cast<u32>((glyph.cellX + glyph.widths.glyphWidth)
	            << 15) / glyph.texWidth;
	u16 texBottom = static_cast<u32>((glyph.cellY + glyph.height)
	             << 15) / glyph.texHeight;
...
void CharWriter::UpdateVertexColor()
{
	...
	mVertexColor.ru = mTextColor.gradationMode != GRADMODE_H ? mTextColor.start
	                                                         : mTextColor.end;
	mVertexColor.ld = mTextColor.gradationMode != GRADMODE_V ? mTextColor.start
	                                                         : mTextColor.end;
	mVertexColor.rd = mTextColor.gradationMode == GRADMODE_NONE
	                    ? mTextColor.start
	                    : mTextColor.end;
