...
{
	return strncmp(reinterpret_cast<char const *>(name1),
	                    reinterpret_cast<char const *>(name2), 16)
	    == 0;
}
...
bool TestFileHeader(res::BinaryFileHeader const &fileHeader, byte4_t testSig)
{
...
void InitGXTexObjFromTPL(GXTexObj *to, TPLPalette *pal, u32 id)
{
	...
	GXBool const mipMap = BOOLIFY_TERNARY_TYPE(
	...
	GXInitTexObjLOD(to, tdp->textureHeader->minFilter,
	                ...
	                tdp->textureHeader->maxLOD, tdp->textureHeader->LODBias,
