...
#include "homebuttonLib/nw4hbm/ut/ResFont.h"
#include "homebuttonLib/nw4hbm/ut/RuntimeTypeInfo.h"
#include "homebuttonLib/nw4hbm/ut/TagProcessorBase.h"
#include "homebuttonLib/nw4hbm/ut/TextWriterBase.h"
#include <types.h>
#include <string.h>
#include <wchar.h>
#include <new.h>
...
namespace
{
	...
	int CalcLineStrNum(f32 *pWidth, ut::TextWriterBase<CharT> *pTextWriter,
	                   CharT const *str, int length, f32 maxWidth,
	                   bool *pbOver);

	template <typename CharT>
	int CalcLineRectImpl(ut::Rect *pRect,
	                     ut::TextWriterBase<CharT> *pTextWriter,
	                     CharT const *str, int length, f32 maxWidth,
	                     bool *pbOver);

	template <typename CharT>
	void CalcStringRect(ut::Rect *pRect, ut::TextWriterBase<CharT> *pTextWriter,
	                    CharT const *str, int length, f32 maxWidth);

	template <typename CharT>
	void CalcStringRectImpl(ut::Rect *pRect,
	                        ut::TextWriterBase<CharT> *pTextWriter,
	                        CharT const *str, int length, f32 maxWidth);
...
template <typename CharT>
int CalcLineStrNum(f32 *pWidth, ut::TextWriterBase<CharT> *pTextWriter,
                   CharT const *str, int length, f32 maxWidth, bool *pbOver)
{
	ut::Rect rect;
...
template <typename CharT>
int CalcLineRectImpl(ut::Rect *pRect, ut::TextWriterBase<CharT> *pTextWriter,
                     CharT const *str, int length, f32 maxWidth, bool *pbOver)
{
	...
	*pbOver = false;
	...
	for (u16 code = reader.Next();
	     ...
	     prStrPos = static_cast<CharT const *>(reader.GetCurrentPos()),
	              code = reader.Next(), prMaxRect = *pRect)
	{
		if ((int)code < L' ')
		{
			...
			operation =
				pTextWriter->GetTagProcessor().CalcRect(&rect, code, &context);
                        ...
                        CharT const *str, int length, f32 maxWidth)
{
	...
	do
	{
		...
		bool bOver;
		int read =
			CalcLineRectImpl(&rect, pTextWriter, pos, remain, maxWidth, &bOver);
...
TextBox::TextBox(u16 allocStrLen, wchar_t const *str, u16 strLen,
                 ut::Font const *pFont)
{
	Init(allocStrLen);
	...
	Pane(pBlock)
{
	...
	char const *fontName = detail::ConvertOffsToPtr<char>(
		fonts, fonts[pBlock->fontIdx].nameStrOffset);
	...
	if (ut::Font const *pFont = resBlockSet.pResAccessor->GetFont(fontName))
	{
		mpFont = pFont;
				 ...
				 ResourceAccessor::SIGNATURE_FONT, fontName, nullptr))
	{
...
void TextBox::DrawSelf(DrawInfo const &drawInfo)
{
	...
	for (int remain = mTextLen; remain > 0;)
	{
		f32 lineWidth;
		bool bOver;
		int lineStrNum = CalcLineStrNum(&lineWidth, &writer, strPos, remain,
		                                mSize.width, &bOver);
...
ut::Rect TextBox::GetTextDrawRect(ut::TextWriterBase<wchar_t> *pWriter) const
{
	...
	textRect.MoveTo(
		basePt.x + (mSize.width - textRect.GetWidth()) * GetTextMagH(),
