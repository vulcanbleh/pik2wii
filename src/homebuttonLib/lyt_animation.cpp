...
f32 GetHermiteCurveValue(f32 frame, res::HermiteKey const *keyArray,
                         u32 keySize)
{
	if (keySize == 1 || frame <= keyArray[0].frame)
		return keyArray[0].value;
                    ...
                    u32 const *animTargetOffsets, f32 frame)
{
	for (int i = 0; i < pAnimInfo->num; ++i)
	{
		res::AnimationTarget const *pAnimTarget =
			detail::ConvertOffsToPtr<res::AnimationTarget>(
				pAnimInfo, animTargetOffsets[i]);

		res::HermiteKey const *keys = detail::ConvertOffsToPtr<res::HermiteKey>(
                       ...
                       u32 const *animTargetOffsets, f32 frame)
{
	for (int i = 0; i < pAnimInfo->num; ++i)
	{
		...
		res::StepKey const *keys = detail::ConvertOffsToPtr<res::StepKey>(
			pAnimTarget, pAnimTarget->keysOffset);

		pPane->SetVisible(GetStepCurveValue(frame, keys, pAnimTarget->keyNum)
		                  != 0);
	}
}
	...
	for (int i = 0; i < pAnimInfo->num; ++i)
	{
		...
		if (pAnimTarget->id < pMaterial->GetTexSRTCap())
		{
			res::HermiteKey const *keys =
				detail::ConvertOffsToPtr<res::HermiteKey>(
					pAnimTarget, pAnimTarget->keysOffset);
	...
	for (int j = 0; j < pAnimInfo->num; ++j)
	{
		    ...
		    && !pAnimTarget->target)
		{
			...
			pMaterial->SetTextureNoWrap(
				pAnimTarget->id, static_cast<TPLPalette *>(tpls[fileIdx]));
		}
	}
}
                                     ...
                                     ResourceAccessor *pResAccessor)
{
	...
	if (pRes->fileNum)
	{
		mpFileResAry = static_cast<void **>(
			Layout::AllocMemory(sizeof *mpFileResAry * pRes->fileNum));
		...
		if (mpFileResAry)
		{
			...
			for (int i = 0; i < mpRes->fileNum; ++i)
			{
				mpFileResAry[i] = pResAccessor->GetResource(
					...
					detail::GetStrTableStr(fileNameOffsets, i), nullptr);
			}
		}
	}
	...
	if (mAnimLinkAry)
	{
		...
		memset(reinterpret_cast<void *>(mAnimLinkAry), 0,
...
void AnimTransformBasic::Bind(Pane *pPane, bool bRecursive)
{
	...
	for (u16 i = 0; i < mpRes->animContNum; ++i)
	{
		res::AnimationContent const *pAnimCont =
			detail::ConvertOffsToPtr<res::AnimationContent>(mpRes,
		                                                    animContOffsets[i]);

		if (pAnimCont->type == res::AnimationContent::ACType_Pane)
		{
			Pane *pFindPane =
				pPane->FindPaneByName(pAnimCont->name, bRecursive);
		...
		{
			Material *pFindMat =
				pPane->FindMaterialByName(pAnimCont->name, bRecursive);
...
void AnimTransformBasic::Bind(Material *pMaterial)
{
	...
	for (u16 i = 0; i < mpRes->animContNum; ++i)
	{
		...
		if (pAnimCont->type == res::AnimationContent::ACType_Material
		    && detail::EqualsMaterialName(pMaterial->GetName(),
		                                  pAnimCont->name))
		{
			mAnimLinkAry[i].SetAnimTransform(this, i);
...
void AnimTransformBasic::Animate(u32 idx, Pane *pPane)
{
	...
	{
		res::AnimationInfo const *pAnimInfo =
			detail::ConvertOffsToPtr<res::AnimationInfo>(pAnimCont,
		                                                 animInfoOffsets[i]);
...
void AnimTransformBasic::Animate(u32 idx, Material *pMaterial)
{
	...
	for (int i = 0; i < pAnimCont->num; ++i)
	{
		...
		case res::AnimationInfo::SIGNATURE_MATERIAL_COLOR_INFO:
			AnimateMaterialColor(pMaterial, pAnimInfo, animTargetOffsets,
			                     mFrame);
			break;
		...
		case res::AnimationInfo::SIGNATURE_MATERIAL_TEXTURE_PATTERN_INFO:
			if (mpFileResAry)
			{
				AnimateTexturePattern(pMaterial, pAnimInfo, animTargetOffsets,
				                      mFrame, mpFileResAry);
			}
