...
void CalcTexRatio(u8 *dstTexRatios, float const *srcTexRatios,
                  int texNum)
{
...
{
	return CalcOffsetTexCoordGenAry(bitGXNums)
	     + sizeof(TexCoordGen) * bitGXNums.texCoordGen;
}

u32 CalcOffsetMatColAry(detail::BitGXNums const &bitGXNums)
{
	return CalcOffsetChanCtrlAry(bitGXNums)
	     + sizeof(ChanCtrl) * bitGXNums.chanCtrl;
}

u32 CalcOffsetTevSwapAry(detail::BitGXNums const &bitGXNums)
{
	return CalcOffsetMatColAry(bitGXNums)
	     + sizeof(ut::Color) * bitGXNums.matCol;
}
...
{
	return CalcOffsetGetAlphaCompare(bitGXNums)
	     + sizeof(AlphaCompare) * bitGXNums.alpComp;
}

u32 CalcOffsetIndirectStageAry(detail::BitGXNums const &bitGXNums)
{
	return CalcOffsetBlendMode(bitGXNums)
	     + sizeof(BlendMode) * bitGXNums.blendMode;
}

u32 CalcOffsetIndTexSRTAry(detail::BitGXNums const &bitGXNums)
{
	return CalcOffsetIndirectStageAry(bitGXNums)
	     + sizeof(IndirectStage) * bitGXNums.indStage;
}

u32 CalcOffsetTevStageAry(detail::BitGXNums const &bitGXNums)
{
	return CalcOffsetIndTexSRTAry(bitGXNums)
	     + sizeof(TexSRT) * bitGXNums.indSRT;
}
...
Material::Material(res::Material const *pRes, ResBlockSet const &resBlockSet)
{
	...
	if (mpGXMem)
	{
		...
		if (texMapNum != nullptr)
		{
			res::Texture const *textures =
			...
			for (u8 i = 0; i < mGXMemNum.texMap; ++i)
			{
				...
				void *pTplRes = resBlockSet.pResAccessor->GetResource(
					ResourceAccessor::SIGNATURE_TEXTURE_IMAGE, fileName,
					nullptr);
		...
		if (indTexSRTNum)
		{
			...
			TexSRT const *pResIndMtx =
				detail::ConvertOffsToPtr<TexSRT>(pRes, resOffs);
		...
		if (tevStageNum)
		{
			...
			TevStage const *pResTevStg =
				detail::ConvertOffsToPtr<TevStage>(pRes, resOffs);
                            ...
                            bool allocAlpComp, bool allocBlendMode)
{
	...
	mpGXMem = Layout::AllocMemory(
	      sizeof(ut::Color)          * (matColNum
	+ /*  sizeof(ChanCtrl)         * */ chanCtrlNum
	...
	+     sizeof(GXTexObj)         *    texMapNum
	+     sizeof(TexSRT)           *    indSRTNum
...
{
	return detail::ConvertOffsToPtr<GXTexObj>(mpGXMem,
	                                          CalcOffsetTexMapAry(mGXMemCap));
}
...
{
	static GXTevKColorSel kColSels[8] = {GX_TEV_KCSEL_K3_A, GX_TEV_KCSEL_K3_B,
	                                     GX_TEV_KCSEL_K3_G, GX_TEV_KCSEL_K3_R,
	                                     GX_TEV_KCSEL_K2_A, GX_TEV_KCSEL_K2_B,
	                                     GX_TEV_KCSEL_K2_G, GX_TEV_KCSEL_K2_R};

	static GXTevKAlphaSel kAlpSels[8] = {GX_TEV_KASEL_K3_A, GX_TEV_KASEL_K3_B,
	                                     GX_TEV_KASEL_K3_G, GX_TEV_KASEL_K3_R,
	                                     GX_TEV_KASEL_K2_A, GX_TEV_KASEL_K2_B,
	...
	if (IsChanCtrlCap())
	{
		...
		GXSetChanCtrl(GX_COLOR0, false, GX_SRC_REG, chanCtrls->GetColorSrc(),
		              GX_LIGHT_NULL, GX_DF_NONE, GX_AF_NONE);
		GXSetChanCtrl(GX_ALPHA0, false, GX_SRC_REG, chanCtrls->GetAlphaSrc(),
		              GX_LIGHT_NULL, GX_DF_NONE, GX_AF_NONE);
		...
		for (int i = 0; i < mGXMemNum.texMap; ++i)
		{
			...
			if (texFmt == GX_TF_C4 || texFmt == GX_TF_C8)
			{
				...
				if (pClutHeader)
				{
					...
					GXInitTlutObj(&tlutObj, pClutHeader->data,
					              pClutHeader->format, pClutHeader->numEntries);
	...
	if (mGXMemNum.tevStage)
	{
		...
		for (int i = 0; i < mGXMemNum.tevStage; ++i)
		{
			...
			GXSetTevColorOp(
				tevStage, tevStages[i].GetColorOp(),
			...
			GXSetTevAlphaOp(
				tevStage, tevStages[i].GetAlphaOp(),
			...
			GXSetTevIndirect(
				tevStage, tevStages[i].GetIndStage(),
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					for (int i = 0; i < mGXMemNum.texMap; ++i)
					{
						...
						GXTevColorArg colDIn =
							i == 0 ? GX_CC_ZERO : GX_CC_CPREV;
						GXTevAlphaArg alpDIn =
							i == 0 ? GX_CA_ZERO : GX_CA_APREV;
				...
				if (mTevCols[0] != DefaultBlackColor
				    || mTevCols[1] != DefaultWhiteColor)
				{
					GXTevStageID tevStage =
						static_cast<GXTevStageID>(tevStageID);
		...
		for (u8 id = 0; id < tevStageNum; ++id)
		{
			...
			GXSetTevColorOp(tevStage, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, TRUE,
			                GX_TEVPREV);
			GXSetTevAlphaOp(tevStage, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, TRUE,
			                GX_TEVPREV);
...
void Material::UnbindAnimation(AnimTransform *pAnimTrans)
{
	NW4HBM_RANGE_FOR_NO_AUTO_INC (it, mAnimList)
	{
		...
		// nullptr is for Layout::UnbindAllAnimation
