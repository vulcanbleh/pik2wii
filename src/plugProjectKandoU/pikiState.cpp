...
void PikiCarrotState::exec(Piki* piki)
{
	if (piki->isAlive()) {
		...
		mTimeRemaining -= sys->getDeltaTime();
...
void PikiFountainonState::exec(Piki* piki)
{
	...
	if (mTimer >= p1) {
		...
		position.y += factor;
		mTimer += sys->getDeltaTime() * 400.0f;
	} else {
		position = mAvgPosition + mPosDiff * mTimer;
		position.y += otherFactor;
		mTimer += sys->getDeltaTime() * 40.0f;
...
{
	piki->applyAirDrag(sys->getDeltaTime(), mHorizontalDrag, mVerticalDrag);
}
...
{
	if (!mHasAnimStarted) {
		mStartWaitTime -= sys->getDeltaTime();
...
inline void PikiPanicState::checkDemo(Piki* piki)
{
	...
	if (flag != 0 && !playData->isDemoFlag(flag) && gameSystem->isStoryMode() && gameSystem->isFlag(GAMESYS_IsGameWorldActive)) {
		mDemoWaitTime -= sys->getDeltaTime();
...
void PikiHipDropState::exec(Piki* piki)
{
	if (mSubState == 0) {
		...
		piki->mFaceDir += sys->getDeltaTime() * PI / 0.2f;
...
bool PikiFallMeckState::becomePikihead(Piki* piki)
{
	...
	if (GameStat::mePikis >= 99) {
		...
		if (sprout) {
			...
			if (mDoAutoPluck) {
				initArg.mAutopluckTimer = 10.0f + 3.0f * sys->getDeltaTime();
			}
			sprout->init(&initArg);
...
void PikiFlyingState::exec(Piki* piki)
{
	...
	if (mIsFlowerPiki == 0 && piki->getHappa() == Flower && piki->mVelocity.y <= 0.0f) {
		...
		piki->mFaceDir = roundAng(piki->mFaceDir + PI * sys->getDeltaTime() / 0.42f);
		mSlowFallWaitTimer += sys->getDeltaTime();
		...
		piki->mVelocity.y += (randRange + 1.0f) * flowerGravity * sys->getDeltaTime();
	}
	/*
	stwu     r1, -0x60(r1)
	mflr     r0
...
void PikiFlickState::exec(Piki* piki)
{
	...
	if (mState == FLICK_Land) { // piki has landed.
		mKnockDownTimer -= sys->getDeltaTime();
...
void PikiDrownState::exec(Piki* piki)
{
		...
		if (mSubState == 0) {
			...
			case 0:
				...
				if (piki->mVelocity.y < 60.0f) {
					piki->mVelocity.y += 500.0f * sys->getDeltaTime();
				}
	...
	if (mSubState == 1) {
		...
		if (navi) {
			...
			f32 speedDiff   = newSep.normalise();
			f32 modifier    = 15.0f;
			if (mIsCalled) {
				modifier *= 2.0f;
			}

			if (speedDiff > modifier * sys->getDeltaTime()) {
				piki->mTargetVelocity = piki->mTargetVelocity + (newSep * modifier) * sys->getDeltaTime();
			} else {
				piki->mTargetVelocity = piki->mTargetVelocity + newSep * sys->getDeltaTime();
			}
