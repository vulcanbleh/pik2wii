...
void CharWriter::PrintGlyph(f32 x, f32 y, f32 z, const Glyph& rGlyph)
{
	...
	u16 texRight = (rGlyph.cellX + rGlyph.widths.glyphWidth) * 0x8000U / rGlyph.texWidth;
...
void CharWriter::LoadTexture(const Glyph& rGlyph, GXTexMapID slot)
{
	...
	if (loadingTexture != mLoadingTexture) {
		GXTexObj texObj;
		GXInitTexObj(&texObj, rGlyph.pTexture, rGlyph.texWidth, rGlyph.texHeight, rGlyph.texFormat, GX_CLAMP, GX_CLAMP, FALSE);
...
void CharWriter::UpdateVertexColor()
{
	...
	mVertexColor.lu.a = (mVertexColor.lu.a * mAlpha) / 255, mVertexColor.ru.a = (mVertexColor.ru.a * mAlpha) / 255;
