...
inline bool IsEqualTexObj(const GXTexObj& rLhs, const GXTexObj& rRhs) {
    return rLhs.pad[0] == rRhs.pad[0] && rLhs.pad[1] == rRhs.pad[1] &&
           rLhs.pad[2] == rRhs.pad[2] && rLhs.pad[3] == rRhs.pad[3] &&
           rLhs.pad[4] == rRhs.pad[4] && rLhs.pad[5] == rRhs.pad[5] &&
           rLhs.pad[6] == rRhs.pad[6] && rLhs.pad[7] == rRhs.pad[7];
}

inline bool IsEqualTlutObj(const GXTlutObj& rLhs, const GXTlutObj& rRhs) {
    return rLhs.padding[0] == rRhs.padding[0] && rLhs.padding[1] == rRhs.padding[1] &&
           rLhs.padding[2] == rRhs.padding[2];
}
...
public:
    TexCoordScaleState() {
        memset(this, 0, sizeof(TexCoordScaleState));
    }
...
private:
    u32 mUpToDate;                        // _00
    TexMapSize wh[GX_MAX_TEXMAP];         // _04
    TexCoordScale scale[GX_MAX_TEXCOORD]; // _24
...
private:
    GXTexObj mTexObj[GX_MAX_TEXMAP]; // _00
    u8 mValidFlag;                   // _100
    u8 PADDING_0x101;                // _101
    u8 PADDING_0x102;                // _102
...
private:
    GXTlutObj mTlutObj[GX_TLUT7 - GX_TLUT0 + 1]; // _00
    u16 mFlag;                                   // _060
...
private:
    ScnDependentTexMtxFuncType mType[NUM_TEX_MTX]; // _00
};
...
private:
    const ResTevData* mpTevData; // _00
};
...
public:
    PosNrmMtxArrayState()
        : mpViewPosMtxArray(nullptr),
          mpViewNrmMtxArray(nullptr),
          mpViewEnvTexMtxArray(nullptr) {}
    ...
    bool IsValid() const {
        return mpViewPosMtxArray != nullptr;
    }
...
private:
    const math::MTX34* mpViewPosMtxArray;    // _00
    const math::MTX33* mpViewNrmMtxArray;    // _04
    const math::MTX34* mpViewEnvTexMtxArray; // _08
};
...
private:
    struct {
        ScnDependentTexMtxFuncPtr func;         // _00
        ScnDependentTexMtxFuncType type;        // _04
    } mTable[NUM_SCNDEPENDENT_TEXMTX_FUNCTYPE]; // _00
};
...
public:
    ...
    void SetFog(const Fog fog, int id) {
        ...
        if (mCurrentFogIdx == id) {
            if (memcmp(&fog.ref(), &mFogData[id], sizeof(FogData)) == 0) {
                return;
            }
...
private:
    u32 mFlag;                 // _00
    int mCurrentFogIdx;        // _04
    FogData mFogData[NUM_FOG]; // _08
};
...
private:
    LightSetting mLightSetting;                 // _00
    int mCurrentLightSetIdx;                    // _10
    u32 mCurrentMaskDiffColor;                  // _14
    u32 mCurrentMaskDiffAlpha;                  // _18
    u32 mCurrentMaskSpecColor;                  // _1C
    u32 mCurrentMaskSpecAlpha;                  // _20
    s8 mLoadedLightIdx[NUM_LIGHT_IN_LIGHT_SET]; // _24
    LightObj mLightObj[NUM_LIGHT];              // _2C
    AmbLightObj mAmbLightObj[NUM_LIGHT];        // _222C
    LightSetData mLightSetData[NUM_LIGHT_SET];  // _242C
};
...
void LightState::LoadLightSet(int id, u32* pDiffColorMask, u32* pDiffAlphaMask,
    ...
    if (id < 0 || id >= NUM_LIGHT_SET) {
        if (pDiffColorMask != nullptr) {
            *pDiffColorMask = 0;
        }

        if (pDiffAlphaMask != nullptr) {
            *pDiffAlphaMask = 0;
        }

        if (pSpecColorMask != nullptr) {
            *pSpecColorMask = 0;
        }

        if (pSpecAlphaMask != nullptr) {
            *pSpecAlphaMask = 0;
        }
...
private:
    mutable u16 mFlag;                        // _00
    u16 mViewIdx;                             // _02
    mutable math::MTX34 mInvCurrentCameraMtx; // _04
    math::MTX34 mCameraMtx[NUM_CAMERA];       // _34
    math::MTX44 mProjMtx[NUM_CAMERA];         // _634
    math::MTX34 mProjTexMtx[NUM_CAMERA];      // _E34
    math::MTX34 mEnvTexMtx[NUM_CAMERA];       // _1434
};
        ...
        if (srt.IsExist(i)) {
            ...
            if (ident) {
                if (!sPostTexMtx.IsIdentity(i)) {
                    ...
                    GXLoadTexMtxImm(identMtx, i * 3 + GX_PTTEXMTX0, GX_MTX3x4);
                }
            } else {
...
void LoadLightSet(int id, u32* pDiffMask, u32* pSpecMask, AmbLightObj* pAmb) {
    ...
    if (pDiffMask != nullptr) {
        *pDiffMask |= diffAlphaMask;
    }

    if (pSpecMask != nullptr) {
