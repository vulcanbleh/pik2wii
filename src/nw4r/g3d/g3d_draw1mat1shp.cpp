...
void LoadMaterial(ResMat mat, u32 ctrl, Draw1Mat1ShpSwap *pSwap, G3DState::IndMtxOp *pIndMtxOp, bool bIgnoreMaterial) {
    if (!bIgnoreMaterial) {
        ...
        if (pSwap == nullptr || !pSwap->misc.IsValid()) {
        ...
        if (pSwap == nullptr || !pSwap->chan.IsValid()) {
    ...
    if (shp.IsValid() && shp.IsVisible()) {
        if (bFur) {
            ...
            for (u8 chanIdx = 0; chanIdx < 4; chanIdx++) {
                chan.GXGetChanCtrl(targetChan[chanIdx], nullptr, nullptr, &matSrc[chanIdx], nullptr, nullptr, &attnFunc[chanIdx]);
            }
