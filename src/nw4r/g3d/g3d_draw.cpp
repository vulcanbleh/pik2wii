...
G3DState::IndMtxOp* GetIndMtxOp(ResMat mat, ResNode node, ResShp shp) {
    ...
    for (i = 0; i < GX_ITM_2 - GX_ITM_0 + 1; i++) {
        ...
        if (pLight != nullptr && pLight->IsEnable()) {
            math::VEC3 lightVec;
            ...
            rOp.SetNrmMapMtx(id, nullptr, &nrmMtx, method[i]);
        }
    }
...
void SetupDraw1Mat1ShpSwap(Draw1Mat1ShpSwap* pSwap,
    ...
    if (pReplacement->texObjDataArray != nullptr) {
        pSwap->texObj = ResTexObj(&pReplacement->texObjDataArray[id]);
    } else {
        pSwap->texObj = ResTexObj(nullptr);
    }

    if (pReplacement->tlutObjDataArray != nullptr) {
        pSwap->tlutObj = ResTlutObj(&pReplacement->tlutObjDataArray[id]);
    } else {
        pSwap->tlutObj = ResTlutObj(nullptr);
    }

    if (pReplacement->texSrtDataArray != nullptr) {
        pSwap->texSrt = ResTexSrt(&pReplacement->texSrtDataArray[id]);
    } else {
        pSwap->texSrt = ResTexSrt(nullptr);
    }

    if (pReplacement->chanDataArray != nullptr) {
        pSwap->chan = ResMatChan(&pReplacement->chanDataArray[id]);
    } else {
        pSwap->chan = ResMatChan(nullptr);
    }

    if (pReplacement->genModeDataArray != nullptr) {
        pSwap->genMode = ResGenMode(&pReplacement->genModeDataArray[id]);
    } else {
        pSwap->genMode = ResGenMode(nullptr);
    }

    if (pReplacement->matMiscDataArray != nullptr) {
        pSwap->misc = ResMatMisc(&pReplacement->matMiscDataArray[id]);
    } else {
        pSwap->misc = ResMatMisc(nullptr);
    }

    if (pReplacement->pixDLArray != nullptr) {
        pSwap->pix = ResMatPix(&pReplacement->pixDLArray[id]);
    } else {
        pSwap->pix = ResMatPix(nullptr);
    }

    if (pReplacement->tevColorDLArray != nullptr) {
        pSwap->tevColor = ResMatTevColor(&pReplacement->tevColorDLArray[id]);
    } else {
        pSwap->tevColor = ResMatTevColor(nullptr);
    }

    if (pReplacement->indMtxAndScaleDLArray != nullptr) {
        pSwap->indMtxAndScale =
            ResMatIndMtxAndScale(&pReplacement->indMtxAndScaleDLArray[id]);
    } else {
        pSwap->indMtxAndScale = ResMatIndMtxAndScale(nullptr);
    }

    if (pReplacement->texCoordGenDLArray != nullptr) {
        pSwap->texCoordGen =
            ResMatTexCoordGen(&pReplacement->texCoordGenDLArray[id]);
    } else {
        pSwap->texCoordGen = ResMatTexCoordGen(nullptr);
    }

    if (pReplacement->tevDataArray != nullptr) {
        pSwap->tev = ResTev(&pReplacement->tevDataArray[id]);
    } else {
        pSwap->tev = ResTev(nullptr);
    }

     if (pReplacement->flag & 1) {
        pSwap->vtxPosTable = nullptr;
        pSwap->vtxNrmTable = nullptr;
        pSwap->vtxClrTable = nullptr;
    } else {
...
#define pDrawCmd reinterpret_cast<const ResByteCodeData::DrawParams*>(pByteCode)
    ...
    for (; (c = *pByteCode) != ResByteCodeData::END;
        ...
        if (node.IsVisible() && shp.IsVisible()) {
			...
			Draw1Mat1ShpDirectly(mat, shp, nullptr, nullptr, mat == prevMat ? ctrl | RESMDL_DRAWMODE_NOPPCSYNC : ctrl, nullptr,
...
#define pDrawCmd reinterpret_cast<const ResByteCodeData::DrawParams*>(pByteCode)
    ...
    for (; (c = *pByteCode) != ResByteCodeData::END;
        ...
        if (pReplacement->visArray != nullptr) {
        ...
        if (visible && shp.IsVisible()) {
			...
			Draw1Mat1ShpDirectly(
                mat, shp, nullptr, nullptr, mat == prevMat ? ctrl | RESMDL_DRAWMODE_NOPPCSYNC : ctrl, &swap,
                detail::GetIndMtxOp(mat, node, shp)
            );
...
void DrawResMdlLoop(const ResMdl mdl, const detail::workmem::MdlZ *pMdlZArray, u32 numMdlZ, u32 drawMode) {
    ...
    for (i = 0; i < numMdlZ; i++) {
        ...
        if (shp.IsVisible()) {
            ...
            Draw1Mat1ShpDirectly(
                mat, shp, nullptr, nullptr, mat == prevMat ? ctrl | RESMDL_DRAWMODE_NOPPCSYNC : ctrl, nullptr,
                detail::GetIndMtxOp(mat, node, shp)
            );
            prevMat = mat;
        }
...
void DrawResMdlLoop(const ResMdl mdl, const detail::workmem::MdlZ *pMdlZArray, u32 numMdlZ, DrawResMdlReplacement *pReplacement,
    u32 drawMode) 
{
    u32 i;
...
#define pDrawCmd reinterpret_cast<const ResByteCodeData::DrawParams*>(pByteCode)
    ...
    for (; (c = *pByteCode) != ResByteCodeData::END;
        ...
        bool visible;
        if (pReplacement == nullptr || pReplacement->visArray == nullptr) {
            visible = node.IsVisible();
        } else {
            visible = pReplacement->visArray[node.GetID()] != 0;
        }
...
void DrawResMdlDirectly(const ResMdl mdl, const math::MTX34* pViewPosMtxArray,
    ...
    G3DState::SetViewPosNrmMtxArray(nullptr, nullptr, nullptr);
}
