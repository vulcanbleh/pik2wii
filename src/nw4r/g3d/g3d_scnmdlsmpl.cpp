...
ScnMdlSimple* ScnMdlSimple::Construct(MEMAllocator* pAllocator, u32* pSize,
    ...
    if (pAllocator != nullptr) {
        ...
        pSimple = new (pBuffer) ScnMdlSimple(
            ...
            viewNrmMtxSize != 0 ? reinterpret_cast<math::MTX33*>(pBuffer + viewNrmMtxOfs) : nullptr,
            viewTexMtxSize != 0 ? reinterpret_cast<math::MTX34*>(pBuffer + viewTexMtxOfs) : nullptr,
            numView,
            viewMtxNum);
        // clang-format on
    }
...
void ScnMdlSimple::ScnMdlSmpl_CalcPosture(u32 param,
    ...
    if (GetScnMdlCallback() != nullptr) {
        ...
        CalcWorld(pWorldMtxArray, GetWldMtxAttribArray(), GetByteCodeCalc(),
                  GetMtxPtr(MTX_WORLD), GetResMdl(), GetAnmObjChr(), nullptr,
                  rootAttr);
    }

    if (GetByteCodeMix() != nullptr) {
...
void ScnMdlSimple::ScnMdlSmpl_G3DPROC_CALC_MAT(u32 param, void* pInfo) {
    ...
    if (GetAnmObjTexPat() != nullptr || GetAnmObjTexSrt() != nullptr ||
        GetAnmObjMatClr() != nullptr) {
...
void ScnMdlSimple::ScnMdlSmpl_G3DPROC_DRAW_OPA(u32 param, void* pInfo) {
    ...
    DrawResMdlDirectly(GetResMdl(), GetViewPosMtxArray(), GetViewNrmMtxArray(),
                       GetViewTexMtxArray(), GetByteCodeDrawOpa(), nullptr, nullptr,
                       drawMode);
...
void ScnMdlSimple::ScnMdlSmpl_G3DPROC_DRAW_XLU(u32 param, void* pInfo) {
    ...
    DrawResMdlDirectly(GetResMdl(), GetViewPosMtxArray(), GetViewNrmMtxArray(),
                       GetViewTexMtxArray(), nullptr, GetByteCodeDrawXlu(), nullptr,
                       drawMode);
...
bool ScnMdlSimple::SetAnmObj(AnmObj* pObj, AnmObjType type) {
    ...
    AnmObjChr* pChr = nullptr;
    AnmObjVis* pVis = nullptr;
    AnmObjMatClr* pClr = nullptr;
    AnmObjTexPat* pPat = nullptr;
    AnmObjTexSrt* pSrt = nullptr;

    switch (type) {
    case ANMOBJTYPE_CHR: {
        if ((pChr = DynamicCast<AnmObjChr>(pObj)) != nullptr) {
            goto _type_chr;
        }
    ...
    case ANMOBJTYPE_VIS: {
        if ((pVis = DynamicCast<AnmObjVis>(pObj)) != nullptr) {
            goto _type_vis;
        }
    ...
    case ANMOBJTYPE_MATCLR: {
        if ((pClr = DynamicCast<AnmObjMatClr>(pObj)) != nullptr) {
            goto _type_clr;
        }
    ...
    case ANMOBJTYPE_TEXPAT: {
        if ((pPat = DynamicCast<AnmObjTexPat>(pObj)) != nullptr) {
            goto _type_pat;
        }
    ...
    case ANMOBJTYPE_TEXSRT: {
        if ((pSrt = DynamicCast<AnmObjTexSrt>(pObj)) != nullptr) {
            goto _type_srt;
        }
