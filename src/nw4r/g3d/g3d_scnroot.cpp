...
void ScnRoot::UpdateFrame() {
    ...
    G3dProc(G3DPROC_UPDATEFRAME, 0, nullptr);
}
    ...
    for (i = 0; i < G3DState::NUM_FOG; i++) {
        ...
        if (fog.IsFogRangeAdjEnable()) {
            if (!fogRangeAdj) {
                ...
                f32 f_width, x;
                cam.GetViewport(&x, nullptr, &f_width, nullptr, nullptr, nullptr);
...
void ScnRoot::CalcWorld() {
    CalcAnmScn();
    G3dProc(G3DPROC_CALC_WORLD, 0, nullptr);
}

void ScnRoot::CalcMaterial() {
    G3dProc(G3DPROC_CALC_MAT, 0, nullptr);
}

void ScnRoot::CalcVtx() {
    G3dProc(G3DPROC_CALC_VTX, 0, nullptr);
}
