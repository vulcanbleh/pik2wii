...
void ResNode::PatchChrAnmResult(ChrAnmResult* pResult) const
{
	...
	if (flags & ChrAnmResult::FLAG_PATCH_TRANS) {
		if (r.flags & ResNodeData::FLAG_TRANS_ZERO) {
			...
			flags &= ~(ChrAnmResult::FLAG_TRANS_ZERO | ChrAnmResult::FLAG_ROT_TRANS_ZERO | ChrAnmResult::FLAG_MTX_IDENT);
	...
	pResult->flags = flags & ~(ChrAnmResult::FLAG_PATCH_SCALE | ChrAnmResult::FLAG_PATCH_ROT | ChrAnmResult::FLAG_PATCH_TRANS);
}
