...
ResTlutObj ResTlutObj::CopyTo(void* pDst) const
{
	...
	if (flag != 0) {
		...
		} else if (!(flag & static_cast<u8>(~(1 << GX_TLUT0 | 1 << GX_TLUT1 | 1 << GX_TLUT2 | 1 << GX_TLUT3 | 1 << GX_TLUT4)))) {
...
ResTexSrt ResTexSrt::CopyTo(void* pDst) const
{
	...
	for (u32 i = 0; flag != 0; flag >>= TexSrt::NUM_OF_FLAGS, i++) {
		...
		math::MTX34Copy(static_cast<math::MTX34*>(&rToEffect.effectMtx), static_cast<const math::MTX34*>(&rFromEffect.effectMtx));
	}
...
{
	if (pMtx != nullptr && id < ResTexSrtData::NUM_OF_TEXTURE) {
...
void ResMatPix::GXSetAlphaCompare(GXCompare comp0, u8 ref0, GXAlphaOp logic, GXCompare comp1, u8 ref1)
{
	...
	// u32 reg = ref0 << GX_BP_ALPHACOMPARE_REF0_SHIFT | ref1 << GX_BP_ALPHACOMPARE_REF1_SHIFT | comp0 << GX_BP_ALPHACOMPARE_COMP0_SHIFT
	//         | comp1 << GX_BP_ALPHACOMPARE_COMP1_SHIFT | logic << GX_BP_ALPHACOMPARE_LOGIC_SHIFT
...
void ResMatPix::GXSetZMode(GXBool test, GXCompare compare, GXBool update)
{
	...
	// u32 reg = test << GX_BP_ZMODE_TEST_ENABLE_SHIFT | compare << GX_BP_ZMODE_COMPARE_SHIFT | update << GX_BP_ZMODE_UPDATE_ENABLE_SHIFT
 ...
 *
 */
bool ResMatPix::GXGetBlendMode(GXBlendMode* pMode, GXBlendFactor* pSrcFactor, GXBlendFactor* pDstFactor, GXLogicOp* pLogic) const
{
	...
	if (pSrcFactor != nullptr) {
		// *pSrcFactor = static_cast<GXBlendFactor>(reg >> GX_BP_BLENDMODE_SRC_FACTOR_SHIFT & GX_BP_BLENDMODE_SRC_FACTOR_LMASK);
	}

	if (pDstFactor != nullptr) {
		// *pDstFactor = static_cast<GXBlendFactor>(reg >> GX_BP_BLENDMODE_DST_FACTOR_SHIFT & GX_BP_BLENDMODE_DST_FACTOR_LMASK);
	}
...
void ResMatPix::GXSetDstAlpha(GXBool enable, u8 alpha)
{
	...
	// u32 reg = alpha << GX_BP_DSTALPHA_ALPHA_SHIFT | enable << GX_BP_DSTALPHA_ENABLE_SHIFT | GX_BP_REG_DSTALPHA << GX_BP_OPCODE_SHIFT;

	// detail::ResWriteBPCmd(pCmd, reg);
}

////////////////////////////////////////////////////////
////////////// TEXTURE ENVIRONMENT COLORS //////////////
////////////////////////////////////////////////////////
 ...
 *
 */
void ResMatChan::GXSetChanCtrl(GXChannelID id, GXBool enable, GXColorSrc ambSrc, GXColorSrc matSrc, GXLightID lightMask,
 ...
 *
 */
bool ResMatChan::GXGetChanCtrl(GXChannelID id, GXBool* pEnable, GXColorSrc* pAmbSrc, GXColorSrc* pMatSrc, GXLightID* pLightMask,
 ...
 *
 */
bool ResMatTexCoordGen::GXGetTexCoordGen2(GXTexCoordID id, GXTexGenType* pFunc, GXTexGenSrc* pParam, GXBool* pNormalize,
                                          u32* pPostMtx) const
{
	...
	if (tgType == GX_XF_TG_REGULAR) {
		...
		if (pParam != nullptr) {
			// *pParam = static_cast<GXTexGenSrc>((regTex >> GX_XF_TEX_BUMPSRCTEX_SHIFT & GX_XF_TEX_BUMPSRCTEX_LMASK) + GX_TG_TEXCOORD0);
		}
 ...
 *
 */
void ResMatTexCoordGen::GXSetTexCoordGen2(GXTexCoordID id, GXTexGenType func, GXTexGenSrc param, GXBool normalize, u32 postMtx)
{
...
void ResTexPlttInfo::BindTex_(const ResTex tex, ResTexObj texObj)
{
	...
	GXInitTexObjLOD(pGXObj, r.min_filt, r.mag_filt, minLod, maxLod, r.lod_bias, r.bias_clamp, r.do_edge_lod, r.max_aniso);
