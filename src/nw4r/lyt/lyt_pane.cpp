...
void Pane::SetName(const char* pName) {
    strncpy(mName, pName, NW4R_RES_NAME_SIZE);
    mName[NW4R_RES_NAME_SIZE] = '\0';
}

void Pane::SetUserData(const char* pUserData) {
    strncpy(mUserData, pUserData, NW4R_LYT_PANE_USERDATA_LEN);
...
void Pane::SetAnimationEnable(const AnimResource &animRes, bool enable, bool recursive) {
    ...
    u8 materialNum = GetMaterialNum();
    for (u8 i = 0; i < materialNum; i++) {
        GetMaterial(i)->SetAnimationEnable(animRes, enable);
    }

	if (recursive) {
        NW4R_UT_LINKLIST_FOREACH (it, mChildList, 
            { it->SetAnimationEnable(animRes, enable, recursive); })
    }
}
