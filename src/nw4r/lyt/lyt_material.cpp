...
Material::Material(const res::Material* pRes, const ResBlockSet& rBlockSet) {
    ...
    u8 indStageNum  = ut::Min<u8>(pRes->resNum.GetIndTexStageNum(), GX_IND_MAX_TEX_STAGE_ID);
    u8 tevStageNum  = ut::Min<u8>(pRes->resNum.GetTevStageNum(), GX_MAXTEVSTAGE);
    // clang-format on
    ...
    if (mpGXMem != nullptr) {
        ...
        if (texMapNum > 0) {
            ...
            for (u8 i = 0; i < texMapNum; num++, i++) {
                ...
                pTexMap[num].ReplaceImage(static_cast<TPLPalette*>(pTexPalette), 0);
                pTexMap[num].SetWrapMode(pResTexMap[i].GetWarpModeS(), pResTexMap[i].GetWarpModeT());
                pTexMap[num].SetFilter(pResTexMap[i].GetMinFilter(), pResTexMap[i].GetMagFilter());
            }
			SetTextureNum(num);
        }
...
void Material::SetName(const char* pName) {
    strncpy(mName, pName, NW4R_LYT_MATERIAL_NAME_LEN);
...
bool Material::SetupGX(bool modulate, u8 alpha) {
    ...
    if (mGXMemNum.texCoordGen > 0) {
        ...
        for (int i = 0; i < mGXMemNum.texCoordGen; i++) {
            ...
            if (pTexCoordGen[i].GetTexGenType() == GX_TEXMAP1  &&
                texMtx != GX_IDENTITY) {
