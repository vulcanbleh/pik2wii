...
void StateDead::init(EnemyBase* enemy, StateArg* Args)
{
	...
	tank->mTargetVelocity.set(0.0f, 0.0f, 0.0f);
	tank->deathProcedure();
...
void StateMove::exec(EnemyBase* enemy)
{
	...
	tank->mWalkTimer += sys->getDeltaTime();
...
void StateMoveTurn::init(EnemyBase* enemy, StateArg* stateArg)
{
	...
	tank->mGoalPosition.set(CG_GENERALPARMS(tank).mTerritoryRadius() * sinf(tank->mTurnGoalDir) + homePos.x, homePos.y,
...
void StateMoveTurn::exec(EnemyBase* enemy)
{
	...
	if (EnemyFunc::isStartFlick(tank, false) || tank->isAttackable(false) || isAngleWithin(deltaDir, 10.0f)) {
		tank->finishMotion();
		tank->setAnimSpeed(60.0f);
	} else {
		f32 sightRad     = CG_GENERALPARMS(tank).mSightRadius.mValue;
...
void StateChaseTurn::exec(EnemyBase* enemy)
{
	...
	if (target) {
		...
		Vector3f targetPos = tank->mGoalPosition;
		if (tank->turnToTarget(targetPos, CG_GENERALPARMS(tank).mTurnSpeed(), CG_GENERALPARMS(tank).mMaxTurnAngle(), 10.0f)) {
			tank->finishMotion();
		}
	}
