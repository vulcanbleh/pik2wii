...
{
	JPABaseShape* shape = work->mResource->getBsp();
	s32 tick            = shape->getClrAnmMaxFrm() + 1;
	work->mClrKeyFrame = (ptcl->mAge + shape->getClrLoopOfst(ptcl->mAnmRandom)) % tick;
}
...
{
    const GXTevColorArg* colorArg = st_ca[(mData->mFlags >> 0x0F) & 0x07];
    const GXTevAlphaArg* alphaArg = st_aa[(mData->mFlags >> 0x12) & 0x01];
    GXSetBlendMode(st_bm[mData->mBlendModeCfg & 0x03], st_bf[(mData->mBlendModeCfg >> 2) & 0x0F], st_bf[(mData->mBlendModeCfg >> 6) & 0x0F], st_lo[(mData->mBlendModeCfg >> 10) & 0x0F]);
    GXSetZMode(mData->mZModeCfg & 0x01, st_c[(mData->mZModeCfg >> 1) & 0x07], (mData->mZModeCfg >> 4) & 0x01);
    GXSetAlphaCompare(st_c[mData->mAlphaCompareCfg & 0x07], mData->mAlphaRef0, st_ao[(mData->mAlphaCompareCfg >> 3) & 0x03], st_c[(mData->mAlphaCompareCfg >> 5) & 0x07], mData->mAlphaRef1);
    ...
    GXSetZCompLoc((mData->mZModeCfg >> 5) & 0x01);
}
