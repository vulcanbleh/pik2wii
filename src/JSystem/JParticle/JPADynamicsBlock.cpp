...
void JPAVolumeCircle(JPAEmitterWorkData* workData)
{
	...
	if (workData->mEmitter->checkDynFlag(JPADYN_FixedInterval)) {
		...
		theta = theta * workData->mVolumeSweep;
		workData->mVolumeEmitIdx++;
	} else {
	...
	distance = workData->mVolumeSize * (workData->mVolumeMinRad + distance * (1.0f - workData->mVolumeMinRad));
	workData->mVolumePos.set(distance * JMASSin(theta), 0.0f, distance * JMASCos(theta));
...
void JPAVolumeSphere(JPAEmitterWorkData* workData)
{
	...
	factor = workData->mVolumeSize * (workData->mVolumeMinRad + (factor * (1.0f - workData->mVolumeMinRad)));

	workData->mVolumePos.set((factor * JMASCos(phi)) * JMASSin(theta), -factor * JMASSin(phi), (factor * JMASCos(phi)) * JMASCos(theta));
	workData->mVelOmni.mul(workData->mVolumePos, workData->mGlobalScl);
	workData->mVelAxis.set(workData->mVolumePos.x, 0.0f, workData->mVolumePos.z);
}
...
void JPAVolumeCylinder(JPAEmitterWorkData* workData)
{
	...
	workData->mVolumePos.set(factor * JMASSin(angle), workData->mVolumeSize * workData->mEmitter->getRandZP(), factor * JMASCos(angle));
	workData->mVelOmni.mul(workData->mVolumePos, workData->mGlobalScl);
	workData->mVelAxis.set(workData->mVolumePos.x, 0.0f, workData->mVolumePos.z);
}
